...
describe("findFiltered", function () {
  test("works: filter by handle", async function () {
    let company = await Company.getfiltered({ name: 'c1' });
  ...
  test("works: filter by minEmployees", async function () {
    let company = await Company.getfiltered({ minEmployees: 2 });
    expect(company).toEqual([
      {
        handle: "c3",
        name: "C3",
  ...
  test("works: filter by maxEmployees", async function () {
    let company = await Company.getfiltered({ maxEmployees: 2 });
    expect(company).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: No companies with those values", async function () {
    ...
    let response = await Company.getfiltered({ name: "D" });
